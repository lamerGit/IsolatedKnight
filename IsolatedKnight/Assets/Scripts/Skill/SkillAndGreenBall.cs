...
public class SkillAndGreenBall : Poolable
{
    int _damage;
    Vector3 _dir;
    float _speed;
    ...
    {
        get { return _deSpawnTimer; }
        set { _deSpawnTimer = value; }
    }
    ...
    float CurrentDeSpawnTimer
    {
        ...
        set
        {
            _currentDeSpawnTimer = Mathf.Clamp(value, 0.0f, DeSpawnTimer);
            if (_currentDeSpawnTimer == DeSpawnTimer)
            {
    ...
    private void Update()
    {
        ...
        {
            if (CurrentDeSpawnTimer < DeSpawnTimer)
            {
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Enemy") && Managers.GameManager.State == GameState.Nomal)
        {
            ...
            other.GetComponent<EnemyBase>().OnPartnerDamage(Damage, DamageType.GreenBall);
